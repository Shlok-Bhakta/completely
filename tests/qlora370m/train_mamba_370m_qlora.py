...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('training_370m.log'),
...
if resume_checkpoint:
    ...
    logger.info(f"Loading {MODEL_ID} in fp16...")
...
logger.info(f"Total parameters: {num_params:.1f}M, Trainable: {trainable_params:.1f}M")
...
def save_checkpoint(step, avg_loss, reason="scheduled"):
    global checkpoint_losses, model, optimizer, scaler
    ...
    merged_model.save_pretrained(ckpt_path)
...
logger.info(f"QLoRA: r=32, alpha=64, 4-bit NF4 quantization")
